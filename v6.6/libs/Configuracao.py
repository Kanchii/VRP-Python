class Configuracao:
    ...
    def gera_Rotas(self):
        ...
        rotas = [Veiculo(i, i / Global.num_Depositos) for i in range(Global.num_Veiculos)]
    ...
    def gera_Rotas_Artigo_Journal(self):
        ...
        conf_Veiculos = self.conf[Global.num_Clientes:]
        matriz_Prioridade = self.gera_Matriz_Prioridade(Global.clientes, conf_Veiculos)
        rotas = [Veiculo(i, i / (Global.num_Veiculos / Global.num_Depositos)) for i in range(Global.num_Veiculos)]
        ...
        for i in range(Global.num_Clientes):
            ...
            for j in range(Global.num_Veiculos):
                ...
                if(tmp != -1):
                    ...
                    break
        for i in range(Global.num_Veiculos):
            rotas[i].add(Global.depositos[rotas[i].idDeposito])
            if(len(rotas[i].clientes) > 2):
                rotas[i].fitness += Global.veiculos_Capacidades[i][1]
        return rotas
