...
if __name__ == '__main__':
    start = time.time()
    time.clock()
    capacidade_max, qtd_clientes, demanda_clientes, maxi_caminhoes, posicoes, matriz_distancias = init_Instancia("Entradas/entrada-2")
    particles = []
    lastTime = 0
    lastIte = 0
    mediaFit = 0
    ...
    for t in range(10):
        ...
        for i in range(MAX_ITERACOES):
            ...
            if(i % 25 == 0):
                for j in range(MAX_PARTICLES):
                    # particles[j].OPT_Pos_Inter(j, posicoes, i + 1, MAX_ITERACOES)
                    # particles[j].OPT_Pos_Intra(j, posicoes, i + 1, MAX_ITERACOES)
                    particles[j].OPT_Pos_Rand(j, posicoes, i, MAX_ITERACOES)
                    mudancaGBest = mudancaGBest or particles[j].updatePBest()
            if(i % 1000 == 0):
                for j in range(MAX_PARTICLES):
                    mudancaGBest = mudancaGBest or particles[j].OPT_PBest_Rand(j, posicoes, -1, -1)
                    mudancaGBest = mudancaGBest or particles[j].OPT_PBest_Intra(j, posicoes, -1, -1)
                    mudancaGBest = mudancaGBest or particles[j].updatePBest()
