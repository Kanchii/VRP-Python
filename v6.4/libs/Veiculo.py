class Veiculo:
    ...
    def quebra_Regra(self, cliente):
        ...
        if(self.carregando + cliente.demanda > Global.veiculos_Capacidades[self.id]):
            return True
    ...
    def simula(self):
            ...
            if(carregando > Global.veiculos_Capacidades[self.id]):
                return -1
            pesoMorto = 0
            tempo_Total = 0
            for i in range(1, len(self.clientes)):
                ...
                if(carregando - self.clientes[i].demanda + self.clientes[i].coleta + pesoMorto > Global.veiculos_Capacidades[self.id]):
                    return -1
