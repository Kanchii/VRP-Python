class Particle():
    def __init__(self):
        ...
        self.lower_Bound_Pos_Vehicle = -200
        self.upper_Bound_Pos_Vehicle = 200
        self.lower_Bound_Vel_Vehicle = 10
        self.upper_Bound_Vel_Vehicle = -10
        ...
        self.position = State(self.lower_Bound_Pos, self.upper_Bound_Pos, self.lower_Bound_Pos_Vehicle, self.upper_Bound_Pos_Vehicle)
        self.velocity = [0] * (Global.num_Clients + 2 * Global.num_Vehicles)
    ...
    def update_Velocity(self, actual_Iteration):
        ...
        for i in range(Global.num_Clients):
            if(self.velocity[i] < self.lower_Bound_Vel or self.velocity[i] > self.upper_Bound_Vel):
                self.velocity[i] = 0
        for i in range(Global.num_Clients, Global.num_Clients + Global.num_Vehicles):
            if(self.velocity[i] < self.lower_Bound_Vel_Vehicle or self.velocity[i] > self.upper_Bound_Vel_Vehicle):
                self.velocity[i] = 0

    def update_Position(self):
        # self.position.update(self.velocity)
        self.position.add_Velocity(self.velocity)
        self.position.set_Limits(self.lower_Bound_Pos, self.upper_Bound_Pos, self.lower_Bound_Pos_Vehicle, self.upper_Bound_Pos_Vehicle)
