class Read_File():
    ...
    def read_File(self, path_Clients, path_Vehicles):
        ...
        vehicles = []
        number_Type_Vehicles = 0
        vehicle_Each_Type_Model = []
        with open(path_Vehicles) as vehicles_File:
            for idx, line in enumerate(vehicles_File.readlines()):
                ...
                '''
                    ...
                    2 -> Cost
                '''
                number_Type_Vehicles += 1
                for i in range(int(data[0])):
                    vehicle = Vehicle(_id = idx,
                                      _capacity=float(data[1]),
                                      _cost=float(data[2]), _depot = clients[0])
                    if(i == 0):
                        vehicle_Each_Type_Model.append(vehicle)
                    vehicles.append(vehicle)
        num_Vehicles = len(vehicles)

        return num_Clients, num_Vehicles, number_Type_Vehicles, vehicle_Each_Type_Model, clients, vehicles
